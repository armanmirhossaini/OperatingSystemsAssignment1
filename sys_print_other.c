...
int print_other_init_module(int pid)
{
  ...
  if (task == NULL)
  {
    printk(KERN_INFO "Could not find the process with that PID\n");
  }
  else
  {
    ...
    printk(KERN_INFO "PID %d's name %s\n", pid, task->comm);
    printk(KERN_INFO "PID %d's parent processes until init is :\n", pid);
